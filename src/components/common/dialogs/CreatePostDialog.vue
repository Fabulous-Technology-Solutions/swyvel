      ...
      <q-card-section>
        <form @submit.prevent="submitForm" class="q-gutter-y-lg">
          ...
          />
              <div v-if="uploadedFiles.length" class="q-mt-md">
                <div v-for="(file, index) in uploadedFiles" :key="index" class="q-mb-md">
                  ...
                  <video v-else-if="file.type.startsWith('video/')" :src="file.url" controls class="rounded-8" style="width: 100%;"></video>
          ...
          <div class="flex justify-between items-center q-gutter-x-md">
            ...
            <q-btn type="submit" no-caps flat label="Confirm & Publish"
              class="col rounded-8 bg-primary text-white q-py-sm"
              color="primary"
              @click="hideDialog"
            />
          </div>
